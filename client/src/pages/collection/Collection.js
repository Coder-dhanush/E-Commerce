    ...
    return (
      ...
      <div className="header">
        <div className="info">
          ...
          <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Quibusdam facere magnam architecto non cupiditate quisquam?</p>
        </div>
        <div className="sort-by">
          <div className="sort-by-container">
            ...
            <select className='select-sort-by' name="sort-by" id="sort-by" onChange={(e)=> setSortBy(e.target.value)}>
             {sortOptions.map(item => <option 
            key={item.sort} value={item.sort}>{item.value}</option>)}
            </select>
          </div>
        </div>
      </div>
